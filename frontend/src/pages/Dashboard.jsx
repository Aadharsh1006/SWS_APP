            ...
            {uploads.map((item) => (
              <div key={item.id} className="rounded-lg border border-gray-200 p-3">
                <div className="flex items-center justify-between gap-2 text-sm">
                  <span className="font-medium text-slate-900">{item.file.name}</span>
                  <span className="text-slate-600">{item.status}</span>
                </div>
                <div className="mt-2 flex h-2 w-full overflow-hidden rounded-full bg-gray-200">
                  <div className="bg-blue-600 transition-all" style={{ width: `${item.progress}%` }} />
                </div>
                <div className="mt-1 text-xs text-slate-500">{item.progress}%</div>
              </div>
            ))}
          </div>
